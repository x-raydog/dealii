...
namespace Step35
{
  ...
  namespace EquationData
  {
    ...
    public:
      ...
      void
      set_component(const unsigned int d);
    ...
    template <int dim>
    void
    Velocity<dim>::value_list(const std::vector<Point<dim>>& points,
                              std::vector<double>&           values,
    ...
    template <int dim>
    void
    Pressure<dim>::value_list(const std::vector<Point<dim>>& points,
                              std::vector<double>&           values,
                              const unsigned int) const
    {
      const unsigned int n_points = points.size();
  ...
  public:
    ...
    void
    run(const bool verbose = false, const unsigned int n_plots = 10);
  ...
  protected:
    ...
    void
    update_pressure(const bool reinit_prec);
  ...
  private:
    ...
    // velocity:
    void
    diffusion_component_solve(const unsigned int d);
  ...
  template <int dim>
  void
  NavierStokesProjection<dim>::create_triangulation_and_dofs(
  ...
  template <int dim>
  void
  NavierStokesProjection<dim>::output_results(const unsigned int step)
  {
  ...
  template <int dim>
  void
  NavierStokesProjection<dim>::assemble_vorticity(const bool reinit_prec)
  {
