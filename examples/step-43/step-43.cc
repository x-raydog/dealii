...
namespace Step43
{
  ...
  private:
    ...
    double
    get_max_u_F_prime() const;
    std::pair<double, double>
    get_extrapolated_saturation_range() const;
    bool
    determine_whether_to_solve_for_pressure_and_velocity() const;
    void
              ...
              for(unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  ...
                  local_matrix(i, j) += porosity * phi_i_s * phi_j_s
  ...
  template <int dim>
  void
  TwoPhaseFlowProblem<dim>::refine_mesh(const unsigned int min_grid_level,
                                        const unsigned int max_grid_level)
  {
