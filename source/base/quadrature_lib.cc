  ...
  Quadrature<1> (n)
{
  ...
  std::vector<long double> points  = internal::QGaussLobatto::compute_quadrature_points(n, 1, 1);
  std::vector<long double> w       = internal::QGaussLobatto::compute_quadrature_weights(points, 0, 0);
...
namespace internal
{
	namespace QT
	{
		std::vector<double>
	get_quadrature_points(const unsigned int n,
			          	const QGaussRadauChebyshev<1>::EndPoint ep)
		{
			...
			for (unsigned short i = 0; i<n; ++i)
				...
				switch (ep)
				{
				case QGaussRadauChebyshev<1>::left:
				{
					points[i] = 1. / 2.*(1. - std::cos(numbers::PI*(1 + 2 * double(i) / (2 * double(n - 1) + 1.))));
					break;
				}

				case QGaussRadauChebyshev<1>::right:
				{
				...
				default:
					Assert(false, ExcMessage("This function can only be called with either "
						"QGaussRadauChebyshev<1>::left or QGaussRadauChebyshev<1>::right as "
						"second argument."));
				}
	...
	get_quadrature_weights(const unsigned int n,
				const QGaussRadauChebyshev<1>::EndPoint ep)
		{
			...
			for (unsigned short i = 0; i<n; ++i)
			{
				...
				if (ep == QGaussRadauChebyshev<1>::left && i == 0)
					weights[i] /= 2.;
				else if (ep == QGaussRadauChebyshev<1>::right && i == (n - 1))
					weights[i] /= 2.;
			}
  ...
  ep (ep)
{
  ...
  std::vector<double> p=internal::QT::get_quadrature_points(n,ep);
  std::vector<double> w=internal::QT::get_quadrature_weights(n,ep);
