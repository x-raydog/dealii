  ...
  if (saved_path != subsection_path)
    {
      ...
      AssertThrow (false, ExcUnbalancedSubsections (filename));
    }
}
                                           ...
                                           const std::string &last_line)
{
  ...
  read_input (input_stream, "input string", last_line);
}
...
void ParameterHandler::parse_input_from_xml (std::istream &in)
{
  ...
  AssertThrow (single_node_tree.get_optional<std::string>("ParameterHandler"),
               ExcMessage ("There is no top-level XML element called "
                           "\"ParameterHandler\"."));
  ...
  AssertThrow (n_top_level_elements == 1,
               ExcMessage ("There are "
                           ...
                           + " top-level elements, but there should only be one."));
                                    ...
                                    const std::string &last_line)
{
  ...
  // Note that, to avoid a stack overflow, we have to explicitly call the
  // function that does the work and *not* a wrapper (which may be virtual and
  // lead us back here)
