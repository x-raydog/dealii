 ...
 *
 * @warning This function only makes sense for array or vector iterators
 * (i.e., the memory between @p begin and @end contains valid objects). This
 * function is intended for use with the various boost vector classes and
 * <code>std::vector</code>, but not, e.g., <code>std::deque</code>.
 *
 * @relates ArrayView
 */
...
/**
 * Create a view that takes a pair of pointers as arguments. ElementType may
 * be const-qualified.
 *
 * @warning this array view only makes sense when @p begin and @p end describe
 * a consecutive in memory range of values (i.e., an array).
 *
 * @relates ArrayView
 */
template <typename ElementType>
